...
export function SignupBasicForm({ form, onNext }: SignupBasicFormProps) {
  ...
  const handleCheckEmail = async () => {
    ...
    try {
      ...
      if (!isAvailable) {
        openAlert({ variant: "warning", title: "이미 사용 중인 이메일입니다." })
      } else {
        setEmailChecked(true)
        openAlert({ variant: "success", title: "사용 가능한 이메일입니다." })
      }
    } catch {
      openAlert({ variant: "warning", title: "이메일 확인 중 오류가 발생했습니다." })
    }
  }
  ...
  const handleCheckNickname = async () => {
    ...
    try {
      ...
      if (isDuplicated) {
        openAlert({ variant: "warning", title: "이미 사용 중인 닉네임입니다." })
      } else {
        setNicknameChecked(true)
        openAlert({ variant: "success", title: "사용 가능한 닉네임입니다." })
      }
    } catch {
      openAlert({ variant: "warning", title: "닉네임 확인 중 오류가 발생했습니다." })
    }
  }

  const handleNext = async () => {
    const valid = await trigger(["email", "password", "passwordConfirm", "name", "nickname"])
    if (!valid) return

    if (!emailChecked) {
      openAlert({ variant: "warning", title: "이메일 중복확인을 해주세요." })
      return
    }
